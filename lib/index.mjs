...
class MmDNS {
    ...
    constructor(api_token, zone_Id) {
        ...
        this._errors = [];
        this._str = `[DNS:OK] All dns process are passed`;
    }
    async list() {
    ...
    async updateCname(subdomain, cnameValue) {
        ...
        if (this._errors.length > 0) {
            this._str = `[DNS:FAIL] Fail dns processes :\n${this._errors.map((m) => `- ${m}\n`)}`;
            return this;
        }
        if (found.record) {
            if (found.record.content && found.record.content === cnameValue) {
                this._str = `[DNS:NOTHING] Cname target want to update "${cnameValue}" is up to date`;
                return this;
            }
            else {
                ...
                this._str = `[DNS:OK] Updated subdomain : "${found.record.name}" with CNAME target : "${cnameValue}"`;
                return this;
            }
        }
        else {
    ...
    async removeCname(subdomain) {
        ...
        if (found.record) {
            ...
            this._str = `[DNS:OK] Removed subdomain : "${found.record.name}"`;
            return this;
        }
        else {
            return this;
...
const updateCachePath = "cache/temp_update.json";
const removeCachePath = "cache/temp_remove.json";
const registerCachePath = "cache/temp_register.json";
...
const cacheDnsPath = "cache/dns.json";
function readCache() {
    return {
        file: async () => {
        ...
        dns: async () => {
            ...
            return (JSON.parse(cacheDNS) ?? []);
        },
    };
}
async function validCacheWrite(main, file, name) {
...
class PrClient {
    ...
    async _pr() {
        const { data: pr } = await this._octokit.request("GET /repos/{owner}/{repo}/pulls/{pull_number}", {
    ...
    async createComment(body) {
        ...
        const { data: comment } = await this._octokit.request("POST /repos/{owner}/{repo}/issues/{issue_number}/comments", {
            owner: this._repo_owner,
            repo: this._repo_name,
    ...
    async createReviewComment(params) {
        ...
        const { data: reviewComment } = await this._octokit.request("POST /repos/{owner}/{repo}/pulls/{pull_number}/comments", {
            owner: this._repo_owner,
            repo: this._repo_name,
            pull_number: parseInt(this._pr_number, 10),
            body: text,
...
async function checkUserLocation(gh_token, repo_owner, repo_name, pr_number) {
...
async function getPrType(gh_token, repo_owner, repo_name, pr_number) {
    const prClient = new PrClient(gh_token, repo_owner, repo_name, pr_number);
    const info = await prClient.prInfo();
    const body = info.body;
    return checkPrRequestBody(body).prt;
}
async function getPrUser(gh_token, repo_owner, repo_name, pr_number) {
    const prClient = new PrClient(gh_token, repo_owner, repo_name, pr_number);
    const info = await prClient.prInfo();
    return info.login;
}
